...
recon_x, dec_layers = build_decoder(state, prob)
...
for layer in enc_layers+dec_layers:
    with tf.variable_scope(layer, reuse=True):
        vars.append(tf.get_variable('kernel'))
        vars.append(tf.get_variable('bias'))
...
train = tf.train.AdamOptimizer(learn_rate).minimize(cost, var_list=vars)

# initializer for network
init = tf.global_variables_initializer()


def iteration_train(iter):
    acc_cost = 0
    acc_cost_y = 0
    acc_cost_div = 0
    acc_cost_recon = 0
...
def iteration_test(iter):
    ...
    for b in range(num_test_batches):
        ...
        if b == 0:
            ...
            plt.savefig("VA_ITER" + str(iter) + ".png")
            plt.pause(0.001)
            plt.close()
    ...
    print("TEST " + str(iter) + " Losses: number " + str(acc_cost_y / num_test_batches)
